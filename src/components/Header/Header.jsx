...
const Header = ({Theme,setTheme}) => {
  ...
  return (
    ...
    className='flex justify-between items-center sticky top-0 px-4 py-4 xl:px-40 lg:px-24 sm:px-12 bg-white/70 backdrop-blur-xl dark:bg-gray-900/70 z-20'>
        <img src={Theme==='dark'?assets.logo_dark:assets.logo} className='sm:w-40 w-32'/>
        ...
        <SignInButton mode='modal'>
          <button className='flex gap-2 max-sm:hidden items-center px-6 py-2 text-sm rounded-3xl bg-[#5044e5] text-white hover:scale-105 transition-all duration-400 ease-in-out'>Login <img src={assets.arrow_icon} className='size-4'/></button>
        </SignInButton>
      </SignedOut>
      <SignedIn>
        ...
        <div className={`max-sm:fixed sm:hidden max-sm:top-0 max-sm:right-0 max-sm:bg-primary max-sm:h-screen ${Sidebar?'max-sm:w-60 max-sm:opacity-100':'max-sm:w-0 max-sm:opacity-0'} max-sm:overflow-hidden max-sm:text-white transition-all duration-700 ease-in-out`}>
          ...
          <div className='max-sm:flex max-sm:flex-col max-sm:items-start max-sm:gap-5 max-sm:mt-20 max-sm:ml-10'>
            <SignedOut>
        <SignInButton mode='modal'>
          <button className='flex justify-center gap-2 items-center px-6 py-2 text-sm rounded-3xl bg-black/75 text-white hover:scale-105 transition-all duration-400 ease-in-out'>Login <img src={assets.arrow_icon} className='size-4'/></button>
        </SignInButton>
      </SignedOut>
      <SignedIn>
        <UserButton />
